...
>
> **✅ GATILHO RESOLVIDO (2026-06-16, peça 1):** gatilho automático *windowed* via **pg_cron + pg_net** (migration `0013_sync_cron.sql`) — a cada 3 min, só dispara `workflow_dispatch` quando há jogo em janela [kickoff-5min, kickoff+3h]; token no Vault; provado em produção (3 ticks consecutivos sem drift, GitHub 204); latência de placar **2-6h → ≤~4 min**. O `schedule` */10 do GitHub (que disparava a cada 2-6h, não 10 min — causa do "demorando demais") segue ativo como backup fraco temporário. **Restam, do plano "Blindado":** peça 2 (backup independente cron-job.org) + peça 3 (heartbeat de detecção); depois **aposentar o `schedule` */10**. Depois: plugar `capture-goals.mjs` no cron; **P3** derivar `tournament_outcome` (campeão=vencedor da final, vice, artilheiro=mais gols) + `teams.advanced_from_group` (times nos 16-avos) → roda `score_bonus()`; **P4** `/admin/bonus` vira read-only/override; melhorar o banner.
>
